# 1 "C:/NXP/S32DS_ARM_v2018.R1/S32DS/S32SDK_S32K14x_EAR_0.8.6/platform/drivers/src/phy/phy_generic.c"
...
# 1 "C:/NXP/S32DS_ARM_v2018.R1/S32DS/S32SDK_S32K14x_EAR_0.8.6/platform/drivers/src/phy/phy_generic.c"
# 18 "C:/NXP/S32DS_ARM_v2018.R1/S32DS/S32SDK_S32K14x_EAR_0.8.6/platform/drivers/src/phy/phy_generic.c"
...
# 19 "C:/NXP/S32DS_ARM_v2018.R1/S32DS/S32SDK_S32K14x_EAR_0.8.6/platform/drivers/src/phy/phy_generic.c" 2
...
# 20 "C:/NXP/S32DS_ARM_v2018.R1/S32DS/S32SDK_S32K14x_EAR_0.8.6/platform/drivers/src/phy/phy_generic.c" 2
...
# 21 "C:/NXP/S32DS_ARM_v2018.R1/S32DS/S32SDK_S32K14x_EAR_0.8.6/platform/drivers/src/phy/phy_generic.c" 2
...
# 23 "C:/NXP/S32DS_ARM_v2018.R1/S32DS/S32SDK_S32K14x_EAR_0.8.6/platform/drivers/src/phy/phy_generic.c" 2
...
#define PRIVATE_AUTONEG_CAPABLE 0x00000001U
#define PRIVATE_AUTONEG_MSCTRL 0x00000002U
#define PRIVATE_LINKUP 0x80000000U
...
static status_t getLinkStatus(uint8_t phy, _Bool* p_linkUp);
...
# 135 "C:/NXP/S32DS_ARM_v2018.R1/S32DS/S32SDK_S32K14x_EAR_0.8.6/platform/drivers/src/phy/phy_generic.c"
static status_t init(uint8_t phy)
{
    status_t phyStatus;
...
static status_t reset(uint8_t phy)
{
    return PHY_RMR(phy, 0U, 0x8000U, 0x8000U);
}
...
static status_t mainFunction(uint8_t phy)
{
    ...
    if (linkUp == 1)
    {
        if ((g_phyConfig[phy].privateData & 0x80000000U) == 0U)
        {
            ...
            g_phyConfig[phy].privateData = g_phyConfig[phy].privateData | 0x80000000U;
        }
    }
    else
    {
...
static status_t getId(uint8_t phy, phy_id_t* id)
{
    ...
    phyStatus = PHY_Read(phy, 2U, &oui3to18);
    if (phyStatus != STATUS_SUCCESS)
    {
        return phyStatus;
    }
    phyStatus = PHY_Read(phy, 3U, &reg);
    if (phyStatus != STATUS_SUCCESS)
    {
        return phyStatus;
    }

    oui19to24 = (uint16_t)((reg & 0xFC00U) >> 10U);
    id->typeNo = (uint8_t)((reg & 0x03F0U) >> 4U);
    id->revisionNo = (uint8_t)(reg & 0x000FU);


    id->oui = (uint32_t) oui19to24;
    id->oui |= ((uint32_t) oui3to18) << 6U;
...
static status_t suspend(uint8_t phy)
{
    return PHY_RMR(phy, 0U, 0x0800U, 0x0800U);
}
...
static status_t resume(uint8_t phy)
{
    return PHY_RMR(phy, 0U, 0, 0x0800U);
}
...
static status_t enableAutoneg(uint8_t phy)
{
    ...
    {
        phyStatus = PHY_RMR(phy, 0U, 0x1000U, 0x1000U);
    }
...
static status_t disableAutoneg(uint8_t phy)
{
    ...
    {
        phyStatus = PHY_RMR(phy, 0U, 0, 0x1000U);
    }
...
static status_t restartAutoneg(uint8_t phy)
{
    ...
    {
        phyStatus = PHY_RMR(phy, 0U, 0x0100U, 0x0100U);
    }
...
static status_t getLinkStatus(uint8_t phy, _Bool* linkUp)
{
    status_t phyStatus;
    uint16_t reg;

    phyStatus = PHY_Read(phy, 1U, &reg);
    if (phyStatus != STATUS_SUCCESS)
    {
        return phyStatus;
    }

    *linkUp = ((reg & 0x0004U) != 0U) ? (1) : (0);
...
static status_t setRole(uint8_t phy, phy_role_t role)
{
    ...
    if ((g_phyConfig[phy].privateData & 0x00000002U) != 0U)
    {
        if (role != PHY_ROLE_AUTO)
        {
            ...
            phyStatus = PHY_RMR(phy, 9U, reg, 0x1000U | 0x1000U);
        }
        else
        {
            phyStatus = PHY_RMR(phy, 9U, 0, 0x1000U);
        }
        if (phyStatus != STATUS_SUCCESS)
        {
            return phyStatus;
...
static status_t setLoopback(uint8_t phy, phy_loopback_t loopbackMode)
{
    ...
    switch (loopbackMode)
    {
        ...
        case PHY_LOOPBACK_EXTERNAL:
            phyStatus = PHY_RMR(phy, 0U, 0x4000U, 0x4000U);
            break;
        case PHY_LOOPBACK_NONE:
            phyStatus = PHY_RMR(phy, 0U, 0, 0x4000U);
            break;
        default:
            phyStatus = STATUS_UNSUPPORTED;
            break;
...
static status_t checkCapabilities(uint8_t phy)
{
    ...
    g_phyConfig[phy].privateData = ((reg & 0x0008U) == 0U) ?
                                    (g_phyConfig[phy].privateData & ~0x00000002U) :
                                    (g_phyConfig[phy].privateData | 0x00000002U);

    if ((reg & (0x0400U | 0x0200U)) != 0U)
    {

        autonegMasterSlave = 1;
    }

    if ((reg & (0x0100U)) != 0U)
    {
        phyStatus = PHY_Read(phy, 15U, &reg);
        if (phyStatus != STATUS_SUCCESS)
        {
            return phyStatus;
        }
        if ((reg & (0x2000U | 0x1000U)) != 0U)
        {
    ...
    g_phyConfig[phy].privateData = (autonegMasterSlave == 1) ?
                                    (g_phyConfig[phy].privateData & ~0x00000002U) :
                                    (g_phyConfig[phy].privateData | 0x00000002U);

    return STATUS_SUCCESS;
}
# 521 "C:/NXP/S32DS_ARM_v2018.R1/S32DS/S32SDK_S32K14x_EAR_0.8.6/platform/drivers/src/phy/phy_generic.c"
static status_t returnNotSupported(uint8_t phy)
{
    (void)(phy);
